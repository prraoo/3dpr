...
if __name__ == '__main__':
	...
	# emap_path = '/HPS/prao2/static00/datasets/Environment-Maps/outdoor-ds-train/'
	...
	use_inset = True
	sub_id = 'ID20029'
	cam_list = [6, 4, 7, 23, 39]

	# output_dir = f'/CT/VORF_GAN5/static00/datasets/evaluation/evaluation-sparse_eval_white/'
	# output_dir = f'/CT/VORF_GAN5/static00/datasets/evaluation/indoor-test-quant-eval/'
	# output_dir = '/CT/VORF_GAN3/work/code/TotalRelighting/FaceOLAT_Weyrich_dir/'
	# output_dir = f'/CT/VORF_GAN5/static00/datasets/evaluation/outdoor-ds-rot-train/'
	# output_dir = f'/CT/VORF_GAN5/static00/datasets/evaluation/indoor-ds-rot-train/'
	# output_dir = f'/CT/VORF_GAN5/static00/datasets/evaluation/evaluation-rotated/'
	output_dir = f'/CT/VORF_GAN5/nobackup/datasets/evaluation/CVPR26-debug/'
	...
	for view_idx, cam_id in enumerate(cam_list):
		...
		save_image_dir_crop_no_inset = os.path.join(output_dir, f'{sub_id}', 'images-crop-no_inset')
		os.makedirs(save_image_dir_crop_no_inset, exist_ok=True)
		...
		# Sample Directions
		light_dirs_npy = dirs.copy()
		uv_idx = get_dir2uv(light_dirs_npy, uv_res=UV_res, dataset_name='mpi')
		
		for emap_idx, emap_name in enumerate(emap_list):
			from_emap = cv2.imread(os.path.join(emap_path, emap_name), -1).astype(np.float32)
			from_emap = cv2.resize(from_emap, (UV_res[1] * 3, UV_res[0] * 3)) # BIG UV
			# from_emap = cv2.resize(from_emap, (UV_res[1] + 1, UV_res[0] + 1)) # SMALL UV
			...
			if not np.isnan(emap.any()):
				...
				if view_idx >= 0:
					...
					print(f"{emap_name} View_idx: {view_idx} im_max: {im_max:0.4f} im_min: {im_min:0.4f}")
				...
				if use_crop_align == True:
					...
					cv2.imwrite(f'{save_image_dir_crop_no_inset}/{cam}_{sub_id}_{emap_name[:-4]}.png', np_img)
					if use_inset:
						...
						# else:
						# 	emap_inset = np.where(emap_inset > 255*0.0, 1*255, 0)
