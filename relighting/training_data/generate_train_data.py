...
if __name__ == '__main__':
	...
	emap_path = '/CT/VORF_GAN4/static00/datasets/env_maps/indoor_ds/'
	...
	use_inset = False
	sub_id = sys.argv[1]
	output_dir = f'/CT/VORF_GAN5/static00/datasets/OLAT_c2-Multiple-IDs/relighting-zspiral_envmap/indoor-0/'
	...
	for view_idx, cam_id in enumerate(cam_list):
		...
		emap_list = emap_config[25*emap_offset:25*(emap_offset+1)]
		...
		for emap_idx, emap_name in enumerate(emap_list):
			...
			if not np.isnan(emap.any()):
				...
				if use_inset:
					...
					emap_inset = cv2.resize(emap_png, (UV_inset_res[1] * scale, UV_inset_res[0] * scale),
