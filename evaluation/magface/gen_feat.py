...
class ImgInfLoader(data.Dataset):
    def __init__(self, ann_file, transform=None):
        ...
        # self.mask_path = '/CT/VORF_GAN4/static00/datasets/OLAT_c2-Multiple-IDs/10001/'
        self.mask_path = '/HPS/prao2/static00/datasets/OLAT_c2-Multiple-IDs/raw/10001/'
        cprint('=> preparing dataset for inference ...')
        self.init()
        self.id_name = os.path.basename(self.imgs[0]).split('_')[1]
    ...
    def __getitem__(self, index):
        ...
        cam = os.path.basename(img_path).split('_')[0]
        # fname = f'Cam06_{id_name}_EMAP-0000.png'
        fname = f'{cam}_{self.id_name}_EMAP-860.png'
        mask = cv2.imread(os.path.join(self.mask_path, self.id_name, 'mask', fname)) / 255
